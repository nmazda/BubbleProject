model = dict(
    type="CascadeRCNN",
    neck=dict(type="FPN", out_channels=256, num_outs=5),
    rpn_head=dict(
        type="RPNHead",
        in_channels=256,
        feat_channels=256,
...
data = dict(
    samples_per_gpu=1,
    workers_per_gpu=1,
    train=dict(pipeline=train_pipeline, type=dataset_type),
    val=dict(pipeline=test_pipeline, type=dataset_type),
    test=dict(pipeline=test_pipeline, type=dataset_type),
)
